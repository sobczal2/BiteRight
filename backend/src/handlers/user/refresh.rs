...
use crate::db::refresh_token::{create_refresh_token, delete_refresh_tokens_for_user, find_refresh_token_by_user_id};
...
pub async fn refresh(
    ...
    let refresh_token_in_db = find_refresh_token_by_user_id(&mut tx, refresh_request.user_id)
        .await?;
    ...
    delete_refresh_tokens_for_user(&mut tx, refresh_request.user_id)
        .await?;

    let jwt =
        generate_jwt(refresh_request.user_id, &app_config.token).map_err(|_| ApiError::internal_error())?;
